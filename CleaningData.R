...
# Illness = read.csv('/Users/janruffner/Desktop/HealthcareFraudCapstoneProject/Illness.csv')
# Illness$Illnxess2 <- gsub('[0-9]+', '', Illness$Illness)
# Illness$ClmDiagnosisCode_1 <- trim(gsub("[a-zA-Z//-]", "", Illness$Illness))
# Illness$Illness <- Illness$Illnxess2
# Illness <- Illness %>% select(-c( "Illnxess2"))
...
#HealthData = merge(HealthData, Illness, by='ClmDiagnosisCode_1')
nrow(HealthData)
...
# The average cost of hospitals seems to be higher
...
# Hospitals with a higher amount of patients tend to be more fradulent
...
# Hospitals with a higher amount of claims tend to be more fradulent


###########
HealthData_2 %>% group_by(PotentialFraud, Provider) %>% 
  summarize(NumberOfPhysicians = n_distinct(AttendingPhysician)) %>% 
  ggplot(aes(NumberOfPhysicians, fill = PotentialFraud, color = PotentialFraud)) +
  geom_density(alpha=0.5) +
  xlab("Number of Physicians working in a hospital") +
  ylab('density')+
  ggtitle("Number of Physicians working in a hospital")
...
########### Fraud by Service (Inpatient, Outpatient, Both Services)
...
library(dplyr)
HealthData_12 <- HealthData_11 %>% group_by(ClmDiagnosisCode_1) %>% mutate(my_ranks = rank(OPAnnualReimbursementAmt), perc = rank(OPAnnualReimbursementAmt)/length(OPAnnualReimbursementAmt))
...
write_csv(HealthData_5, '/Users/janruffner/Desktop/HealthcareFraudCapstoneProject/HealthData.csv')
